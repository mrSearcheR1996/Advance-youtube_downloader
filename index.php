...
<body>
    <div class="container">
        ...
        <?php if($error) :?>
            <div style="color:blue;font-weight: bold;text-align: center"><?php print $error?></div>
        <?php endif;?>

        <?php if (isset($formats)):?>
        <div class="row formSmall">
            <div class="col-lg-3">
                <img with=320 height=240 src="getthumbnail.php?in=<?php echo($thumbnail)?>&title=<?php echo str_replace("&"," ",$title)?>">
            </div>
            <div class="col-lg-9" style="color:blue;font-weight:bold;text-align:right" >
                <?php print $title?>
            </div>
        </div>
                ...
                <table class="table ">
                    ...
                    <?php foreach ($formats as $video) :?>
                        <tr>
                            ...
                            <td><a target="_blank" rel="noopener noreferrer" href="downloader.php?link=<?php print urlencode($video['link'])?>&title=<?php print urlencode($title)?>&type=<?php print urlencode($video['type'])?>&dl=1">Save</a> </td>
                        </tr>
                    <?php endforeach;?>
                </table>
            </div>
                ...
                <table class="table ">
                    ...
                    <?php foreach ($adapativeFormats as $video) :?>
                        <tr>
                            ...
                            <td><?php print $video['quality']?></td>
                            <td><a href="downloader.php?link=<?php print urlencode($video['link'])?>&title=<?php print urlencode($title)?>&type=<?php print urlencode($video['type'])?>">Download</a> </td>
                            <td style="text-align:center" ><a  target="_blank" rel="noopener noreferrer" href="downloader.php?link=<?php print urlencode($video['link'])?>&title=<?php print urlencode($title)?>&type=<?php print urlencode($video['type'])?>&dl=1">Save</a> </td>
                        </tr>
                    <?php endforeach;?>
                </table>
            </div>
